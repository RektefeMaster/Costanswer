  ...
  return (
    <CalculatorPanel title="Investment projection" intro="Starting amount, recurring contributions, and an assumed return. Not a guarantee." toolId="investment" category="money" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={JSON.stringify([principal, contribution, contributionFrequency, contributionTiming, annualReturnPercent, years, compounding])}>
      <div className="calc-form-grid">
        <Field label={t("Starting amount")} htmlFor="inv-principal"><InputShell prefix="$"><input id="inv-principal" type="number" min="0" step="100" inputMode="decimal" value={principal} onChange={(event) => setPrincipal(event.target.value)} /></InputShell></Field>
        <Field label={t("Recurring contribution")} htmlFor="inv-contrib"><InputShell prefix="$"><input id="inv-contrib" type="number" min="0" step="25" inputMode="decimal" value={contribution} onChange={(event) => setContribution(event.target.value)} /></InputShell></Field>
        <Field label={t("Assumed annual return")} htmlFor="inv-return" hint={t("An assumption, not a forecast")}><InputShell suffix="%"><input id="inv-return" type="number" min="0" max="40" step="0.1" inputMode="decimal" value={annualReturnPercent} onChange={(event) => setAnnualReturnPercent(event.target.value)} /></InputShell></Field>
        <Field label={t("Years")} htmlFor="inv-years"><InputShell suffix={t("years")}><input id="inv-years" type="number" min="0" max="80" step="1" inputMode="decimal" value={years} onChange={(event) => setYears(event.target.value)} /></InputShell></Field>
      </div>
      <AdvancedSection
        id="frequency"
      ...
      >
        <div className="calc-form-grid">
          <Field label={t("Compounding")} htmlFor="inv-comp">
          ...
          <Field label={t("Contribution frequency")} htmlFor="inv-freq">
            <span className="input-shell select-shell">
      ...
      <div className="mode-tabs" role="group" aria-label={t("Contribution timing")}>
      ...
      {calculation.result && (
        <div className="calculation-output">
          <PrimaryResult label={t("Estimated ending value")} value={money(calculation.result.value.endingValue)} note="Under the return assumption you entered" tone="mint" />
          <StatGrid items={[
