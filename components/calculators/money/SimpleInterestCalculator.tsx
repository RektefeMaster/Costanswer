  ...
  return (
    <CalculatorPanel title="Simple interest" intro="Interest = principal × annual rate × time. No compounding." toolId="interest" category="money" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={`${principal}|${annualRatePercent}|${years}`}>
      <div className="calc-form-grid">
        <Field label={t("Principal")} htmlFor="si-principal"><InputShell prefix="$"><input id="si-principal" type="number" min="0" step="100" inputMode="decimal" value={principal} onChange={(event) => setPrincipal(event.target.value)} /></InputShell></Field>
        <Field label={t("Annual interest rate")} htmlFor="si-rate"><InputShell suffix="%"><input id="si-rate" type="number" min="0" max="100" step="0.01" inputMode="decimal" value={annualRatePercent} onChange={(event) => setAnnualRatePercent(event.target.value)} /></InputShell></Field>
        <Field label="Time" htmlFor="si-years"><InputShell suffix={t("years")}><input id="si-years" type="number" min="0" max="100" step="0.01" inputMode="decimal" value={years} onChange={(event) => setYears(event.target.value)} /></InputShell></Field>
      </div>
