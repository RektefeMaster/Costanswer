...
export function SquareFootageCalculator() {
  ...
  return (
    <CalculatorPanel title="Square footage" intro="Rectangles added together. Metric input converts through the shared conversion engine." toolId="square-footage" category="home" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={JSON.stringify([unit, spaces])}>
      <div className="mode-tabs" role="group" aria-label={t("Length unit")}>
      ...
      <div className="ingredient-editor area-editor" role="group" aria-label={t("Spaces")}>
        <div className="ingredient-head" aria-hidden="true"><span>Name</span><span>{t("Length")}</span><span>{t("Width")}</span><span /><span /></div>
        {spaces.map((space, index) => (
          <div className="ingredient-row" key={space.id}>
            ...
            <label><span className="sr-only">Space {index + 1} length</span><span className="ingredient-mobile-label" aria-hidden="true">{t("Length")}</span><input id={index === 0 ? 'area-length-0' : undefined} type="number" min="0.01" step="0.1" inputMode="decimal" value={space.length} onChange={(event) => setSpaces((current) => current.map((row) => row.id === space.id ? { ...row, length: event.target.value } : row))} /></label>
            <label><span className="sr-only">Space {index + 1} width</span><span className="ingredient-mobile-label" aria-hidden="true">{t("Width")}</span><input type="number" min="0.01" step="0.1" inputMode="decimal" value={space.width} onChange={(event) => setSpaces((current) => current.map((row) => row.id === space.id ? { ...row, width: event.target.value } : row))} /></label>
            <span />
      ...
      {calculation.result && (
        <div className="calculation-output">
          <PrimaryResult label={t("Total area")} value={`${calculation.result.value.totalSquareFeet.toLocaleString('en-US')} ft²`} note={`${calculation.result.value.totalSquareMeters.toLocaleString('en-US')} m²`} tone="amber" />
