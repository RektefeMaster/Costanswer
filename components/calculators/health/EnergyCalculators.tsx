...
function EnergyFields({
  ...
  return (
    <>
      ...
      <div className="mode-tabs compact-grid" role="group" aria-label={t("Sex used by the equation")}>
      ...
      <div className="calc-form-grid">
        <Field label={t("Age")} htmlFor="energy-age" hint={t("Adult estimate, 18–80")}>
        ...
        <Field label={t("Weight")} htmlFor="energy-weight">
        ...
        <Field label={t("Height")} htmlFor="energy-height">
        ...
        {showActivity && (
          <Field label={t("Activity")} htmlFor="energy-activity">
...
export function BmrCalculator() {
  ...
  return (
    <CalculatorPanel title={t("Estimated BMR")} intro="Mifflin–St Jeor resting-energy estimate for adults. Activity is not applied here." toolId="bmr" category="health" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={JSON.stringify([state.unitSystem, state.sex, state.age, state.weight, state.height])}>
      <EnergyFields {...state} showActivity={false} />
      {calculation.error && <InlineError message={calculation.error} />}
      {calculation.result && (
        <div className="calculation-output">
          <PrimaryResult label={t("Estimated BMR")} value={`${calculation.result.value.bmrKcal.toLocaleString('en-US')} kcal/day`} note="Not a measured metabolic rate." tone="rose" />
...
export function TdeeCalculator() {
  ...
  return (
    <CalculatorPanel title={t("Estimated TDEE")} intro="BMR × a documented activity factor. The factor is a planning assumption." toolId="tdee" category="health" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={JSON.stringify([state.unitSystem, state.sex, state.age, state.weight, state.height, state.activity])}>
      <EnergyFields {...state} />
      {calculation.error && <InlineError message={calculation.error} />}
      {calculation.result && (
        <div className="calculation-output">
          <PrimaryResult label={t("Estimated TDEE")} value={`${calculation.result.value.tdeeKcal.toLocaleString('en-US')} kcal/day`} note={`${ACTIVITY_FACTORS[calculation.result.value.activity].label} × BMR`} tone="rose" />
          <StatGrid items={[
...
export function CalorieCalculator() {
  ...
  return (
    <CalculatorPanel title={t("Estimated daily calories")} intro="Maintenance calories from TDEE, with an optional small planning offset." toolId="calorie" category="health" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={JSON.stringify([state.unitSystem, state.sex, state.age, state.weight, state.height, state.activity, goal])}>
      ...
      {calculation.result && (
        <div className="calculation-output">
          <PrimaryResult label={t("Estimated daily calories")} value={`${calculation.result.value.goalKcal.toLocaleString('en-US')} kcal/day`} note={goal === 'maintain' ? 'Estimated maintenance calories' : 'Includes a small planning offset'} tone="rose" />
          <StatGrid items={[
