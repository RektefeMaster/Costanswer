...
export function PercentageCalculator() {
  ...
  return (
    <CalculatorPanel title={t("Percentage")} intro="Common percent operations and discounts. Each stays a separate calculation." toolId="percentage" category="math" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={`${mode}|${first}|${second}`}>
      <div className="mode-tabs" role="group" aria-label={t("Percentage operation")}>
...
export function PercentChangeCalculator() {
  ...
  return (
    <CalculatorPanel title={t("Percent change")} intro="How much a value rose or fell from an original baseline." toolId="percent-change" category="math" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={`${oldValue}|${newValue}`}>
      <div className="calc-form-grid">
        <Field label={t("Original value")} htmlFor="pc-old"><InputShell><input id="pc-old" type="number" step="0.01" inputMode="decimal" value={oldValue} onChange={(event) => setOldValue(event.target.value)} /></InputShell></Field>
        <Field label={t("New value")} htmlFor="pc-new"><InputShell><input id="pc-new" type="number" step="0.01" inputMode="decimal" value={newValue} onChange={(event) => setNewValue(event.target.value)} /></InputShell></Field>
      </div>
      {calculation.error && <InlineError message={calculation.error} />}
      {calculation.result && (
        <div className="calculation-output">
...
export function FractionCalculator() {
  ...
  return (
    <CalculatorPanel title="Exact fractions" intro="Integer numerator and denominator arithmetic, then GCD simplification." toolId="fraction" category="math" calculationState={calculation.result ? 'complete' : 'invalid'} calculationSignature={`${operation}|${ln}|${ld}|${rn}|${rd}`}>
      ...
      <div className="calc-form-grid">
        <Field label={t("Left numerator")} htmlFor="frac-ln"><InputShell><input id="frac-ln" type="number" step="1" inputMode="numeric" value={ln} onChange={(event) => setLn(event.target.value)} /></InputShell></Field>
        <Field label={t("Left denominator")} htmlFor="frac-ld"><InputShell><input id="frac-ld" type="number" step="1" inputMode="numeric" value={ld} onChange={(event) => setLd(event.target.value)} /></InputShell></Field>
        {operation !== 'simplify' && (
          <>
            <Field label={t("Right numerator")} htmlFor="frac-rn"><InputShell><input id="frac-rn" type="number" step="1" inputMode="numeric" value={rn} onChange={(event) => setRn(event.target.value)} /></InputShell></Field>
            <Field label={t("Right denominator")} htmlFor="frac-rd"><InputShell><input id="frac-rd" type="number" step="1" inputMode="numeric" value={rd} onChange={(event) => setRd(event.target.value)} /></InputShell></Field>
          </>
        )}
      </div>
      {calculation.error && <InlineError message={calculation.error} />}
      {calculation.result && (
        <div className="calculation-output">
          <PrimaryResult label={t("Exact result")} value={`${calculation.result.value.numerator}/${calculation.result.value.denominator}`} note={calculation.result.value.mixed} tone="violet" />
